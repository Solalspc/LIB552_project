...
#         cells_nodes (numpy.ndarray of int): For each cell, the nodes idx (n_cells x cell.n_nodes).
#         cells_edges (numpy.ndarray of int): For each cell, the edges idx (n_cells x cell.n_edges).
#         n_edges (uint): The number of edges.
#         edge (LIB552.Cell): Edge structure.
#         edges_nodes (numpy.ndarray of int): For each edges, the nodes idx (n_edges x edge.n_nodes).
#     """
...
class Mesh:
    ...
    Attributes:
        ...
        n_edges (uint): The number of edges.
        edge (LIB552.Cell): Edge structure.
        edges_nodes (numpy.ndarray of int): For each edge, the nodes idx (n_edges x edge.n_nodes).
        cells_mid_edges (numpy.ndarray of int): Midpoint nodes for each edge in each cell (n_cells x n_edges).
    """
    def __init__(self, dim, nodes, cell, cells_nodes):
        assert (dim in (1, 2))
        self.dim = dim
        ...
        if self.dim == 1:
            ...
            # Update edges calculation considering midpoints
            self.n_edges = self.n_nodes + (self.n_cells * 3) - 3  # Updated for P2 elements
            self.edges_nodes = numpy.empty((self.n_edges, 2), dtype=int)
            self.cells_edges = numpy.empty((self.n_cells, 3), dtype=int)
            nodes_edges = [set() for _ in range(self.n_nodes)]
            cell_nodes = numpy.empty(self.cell.n_nodes, dtype=int)
            edge_nodes = numpy.empty(2, dtype=int)
            k_edge = 0

            # Create midpoints for each edge of each cell
            self.cells_mid_edges = numpy.empty((self.n_cells, 3), dtype=int)
            midpoint_index = self.n_nodes  # Start after the last original node
            
            for k_cell in range(self.n_cells):
                cell_nodes[:] = self.cells_nodes[k_cell]

                # For each edge in the triangle, find or create midpoints
                for k_cell_edge in range(self.cell.n_edges):
                    edge_nodes[:] = cell_nodes[self.cell.get_edge_nodes(k_cell_edge)]

                    # Check if midpoint already exists
                    sorted_edge_nodes = tuple(sorted(edge_nodes))
                    if sorted_edge_nodes not in nodes_edges:
                        # Assign new midpoint node if not already created
                        self.edges_nodes[k_edge] = edge_nodes
                        self.cells_edges[k_cell, k_cell_edge] = k_edge
                        self.cells_mid_edges[k_cell, k_cell_edge] = midpoint_index
                        nodes_edges[edge_nodes[0]].add(k_edge)
                        nodes_edges[edge_nodes[1]].add(k_edge)
                        midpoint_coords = (self.nodes[edge_nodes[0]] + self.nodes[edge_nodes[1]]) / 2
                        self.nodes = numpy.vstack((self.nodes, midpoint_coords))
                        midpoint_index += 1
                        k_edge += 1
                    else:
                        # Use the existing midpoint
                        self.cells_edges[k_cell, k_cell_edge] = k_edge
                        self.cells_mid_edges[k_cell, k_cell_edge] = midpoint_index
...
def create_interval_mesh(x0, x1, n_cells=1):
    ...
    cell = lib.Cell_Line()
    cells_nodes = numpy.empty((n_cells, cell.n_nodes), dtype=int)
...
def create_unit_triangle_mesh():
    ...
    """
    dim = 2
    # Define the triangle nodes
    nodes = numpy.array(
        [[0., 0.],
         [1., 0.],
         [0., 1.]])
    
    # Create a cell for the triangle
    cell = lib.Cell_Triangle()

    # Define the cells_nodes array (triangle with nodes 0, 1, 2)
    cells_nodes = numpy.array(
        [[0, 1, 2]], dtype=int)
    
    # Create the mesh object with midpoints handled by Mesh class
    return lib.Mesh(
        dim=dim,
        nodes=nodes,
        cell=cell,
