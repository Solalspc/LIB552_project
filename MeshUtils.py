...
def field_to_ugrid(field, mesh, finite_element, dof_manager, field_name=None):
    ...
    if (field_dim == 1):
        ...
        for k_dof in range(n_dofs):
            k_cell, k_cell_dof = dof_manager.global_to_local[k_dof] #recupere la cellule et la place dans les dofs de la cellule
            k_cell_node = finite_element.dofs_attachement_idx[k_cell_dof] #recupere le numero du noeud dans la cellule
            k_node = mesh.cells_nodes[k_cell, k_cell_node]  #recupere le numero du noeud dans le maillage global
            k_component = finite_element.dofs_component[k_cell_dof] #recupere le numero du component
            field_vtk[k_node,k_component] = field[k_dof]


    elif (field_dim == 3):
        field_vtk = numpy.zeros((mesh.n_nodes, 3))
        for k_dof in range(n_dofs):
            k_cell, k_cell_dof = dof_manager.global_to_local[k_dof]
            k_cell_node = finite_element.dofs_attachement_idx[k_cell_dof]
            k_node = mesh.cells_nodes[k_cell, k_cell_node]
            k_component = finite_element.dofs_component[k_cell_dof]
            # print("k_component = ",k_component)
            field_vtk[k_node,k_component] = field[k_dof]
